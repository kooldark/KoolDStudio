document.addEventListener('DOMContentLoaded', async () => {
  ...
  const previewGrid = document.getElementById('previewGrid') || null;
  ...
  function loadFromCode(code) {
    try {
      ...
      if (previewSection) updatePreview();
      updateSelectionCounter();
    } catch (e) {
  ...
  try {
    ...
    if (code) {
      ...
      // Update counter if loading from draft
      updateSelectionCounter();
    }
  ...
  clearBtn.onclick = () => {
    if (confirm('Xóa tất cả lựa chọn?')) {
      ...
      document.querySelectorAll('.moodboard-grid a.selected').forEach(el => el.classList.remove('selected'));
  ...
  if (messengerShareBtn) {
    messengerShareBtn.onclick = () => {
      ...
      if (!shareLink) {
        alert('Vui lòng tạo mã moodboard trước');
        return;
      }
